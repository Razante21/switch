export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      const [inscricoes, turmasTodas, polosStatus, esperaGlobal, enderecos] = await Promise.all([
        ecGet('inscricoes'),
        ecGet('turmas'),
        ecGet('polosStatus'),
        ecGet('esperaGlobal'),
  ...
  if (req.method === 'POST') {
    ...
    // Atualizar endereço de um polo
    if (body.polo && body.endereco !== undefined) {
      const atual = await ecGet('enderecos') ?? {};
      atual[body.polo] = body.endereco;
      const ok = await ecSet([{ operation: 'upsert', key: 'enderecos', value: atual }]);
      return res.status(ok ? 200 : 500).json({ success: ok });
    }
